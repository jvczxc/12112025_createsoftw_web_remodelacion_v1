...
if (-not $P) { throw "Missing variable P" }
if (-not $L) { throw "Missing variable L" }
...
function tr([string]$s) {
  ...
  $u = "https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=$L&dt=t&q=$q"
  try {
...
Write-Output ("$P changed=$changed")
